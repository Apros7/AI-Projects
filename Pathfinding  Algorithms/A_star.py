...
class AdjustedAstarSolver(BaselineSolver):
    """
    Simply just has more weight on the heuristic rather than the start node
    """
    def __init__(self, start_node, target_node, cols, rows, heuristic_weight = 3) -> None:
    ...
    def random_selection_function(self, avaliable_nodes): 
        distance_to_start_node = [(node, self.get_distance(node, self.start_node)) for node in avaliable_nodes]
        heuristic_to_target_node = [(node, self.get_distance(node, self.target_node)) for node in avaliable_nodes]
        total_cost = [(g[0], g[1] + self.heuristic_weight*h[1]) for g, h in zip(distance_to_start_node, heuristic_to_target_node)]
        return min(total_cost, key=lambda x: x[1])[0]
